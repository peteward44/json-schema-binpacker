...
function traverseObject( output, allIdMap, object, currentId ) {
	...
	if ( entry ) {
		...
		case 'array':
			...
			if ( entry.children ) {
				const keys = Array.from( entry.children.keys() ).map( str => parseInt( str, 10 ) ).sort();
				for ( let i=0; i<object.length; ++i ) {
					const childId = entry.children.get( ( i % keys.length ).toString() );
...
function createType( input, map, elementOut = null ) {
